# Milestone 20: Advanced Physics Mechanics (Motors & Conveyors) - Implementation Plan

This document breaks down the specification for Milestone 20 into logical, actionable implementation phases.
...
2. Append a new entity profile for `conveyor_belt`.
   ...
   - Add a `properties` dictionary with a `speed` attribute (e.g., default `100.0`) and a `direction` attribute (e.g., `"right"`).
...
3. Append a new entity profile for `motor`.
   ...
   - Add a `properties` dictionary with a `motor_speed` attribute (e.g., default `3.14`) and a `direction` attribute (e.g., `"clockwise"`).
...
3. Fetch the horizontal speed using `self.get_property("speed", 100.0)` and `self.get_property("direction", "right")`.
4. Apply this speed to the physical Pymunk shape's `surface_velocity`.
   - Read `direction`. If it is `"left"`, multiply speed by `-1`.
   - Example mapping: `self.shape.surface_velocity = (speed, 0)`.
5. Integrate logic signaling (M17): If the entity has an active/inactive state triggered via `receive_signal` (e.g., from a Basket or Processor), conditionally apply `(speed, 0)` or `(0, 0)`.
...
4. The `SimpleMotor` drives the relative rotation. Use `self.get_property("motor_speed", 3.14)` for the motor's defined rate, and invert the rate (multiply by `-1`) if `direction` is `"counter-clockwise"`.
...
**Goal:** Allow users to tweak speeds and directions in real-time.
...
3. Re-apply the updated values to the Pymunk Engine.
   - For Conveyors: update `self.shape.surface_velocity = (new_speed, 0)` checking the new direction override.
   - For Motors: update the `rate` property of the `SimpleMotor` constraint checking the new direction override.
**Testing:** Place a Conveyor. Select it in EDIT mode. Change its `direction` to `"left"` in the UI. Hit play. Verify objects are now pushed in the opposite direction.
