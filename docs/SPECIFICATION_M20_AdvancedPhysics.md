# Milestone 20: Advanced Physics Mechanics (Motors & Conveyors) - Specification

## 1. Overview
Milestone 20 introduces driven, continuous motion mechanics to the "Incredible Machines" sandbox environment. By adding active mechanical components like Conveyor Belts and Motors (Spinning Gears/Wheels), players will be able to construct more elaborate, dynamic Rube Goldberg-style contraptions that can physically transport data payloads across the simulation. This extends the sandbox beyond rudimentary gravity-driven physics, allowing for automated transport and rotational drive systems. 
...
- **Integrate with Signals**: Connect these mechanical components to the Logic Wiring system (Milestone 17) to allow circuit-based toggling (On/Off behavior) via active components like Baskets or Processors.
...
- **Shape & Type**: Motors will be implemented as dynamic bodies (typically circles or polygons representing gears) that rotate constantly.
...
Both Conveyor Belts and Motors will be capable of receiving logic signals (e.g., from a Basket or a Processor).
...
- [ ] Both `speed`/`direction` (Conveyor) and `motor_speed`/`direction` (Motor) can be modified on individual instances using the Property Editor.
- [ ] Logic signals (e.g., from a Basket or Processor ingest) can successfully start or stop the motion of a wired Conveyor or Motor.
- [ ] The save/load system correctly persists the customized speeds and logic connections.
